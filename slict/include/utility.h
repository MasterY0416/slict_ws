...
struct LidarCoef
{
    double   t   = -1;        // Time stamp     时间戳
    int      ptIdx;               // Index of points in the pointcloud            点云中点的索引
    Vector3d f;                // Coordinate of point in body frame       身体坐标系中的点坐标
    Vector3d fdsk;        // Coordinate of point in body frame, deskewed          身体坐标系中的点坐标，校正后
    Vector3d finW;        // Coordinate of point in world frame, for vizualization       世界框架中点的坐标，用于可视化
    Vector4d n;              // Score-multipled Hesse normal vector    得分倍增的 Hesse 法向量
    int scale;                   // Scale at which the associated surfel is at     关联面元所在的比例
    int surfNp;                // Number of points in the associated surfel      关联面元中的点数
    double plnrty =  0;   // Surfel planarity     平面度
    double d2P    = -1;    // Distance to the plane    到平面的距离
    double u      = -1;       // Start time of the containing segment   包含段的开始时间
    double s      = -1;       // Scaled time within the used segment   使用段内的缩放时间
    double dt     = -1;     // Time interval of the containing segment  包含段的时间间隔

    deque<ufo::map::Node> ufonode;
    // Clear up the buffer in the destructor   在析构函数中清除缓冲区
