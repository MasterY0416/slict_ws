...
public:
    ...
    bool ProcessData()
    {
        while (ros::ok())
        {
            ...
            for(int i = 0; i < SwImuBundle.back().size(); i++)
            {
                ...
                //IMU数据不中断（任何子段都应该有IMU数据）
                // printf("Step: %2d / %2d. imuSubSeq.size(): %d\n", i, SwImuBundle.back().size(), imuSubSeq.size());
                ROS_ASSERT(!imuSubSeq.empty());

                // ASSUMPTION: an IMU sample is interpolated at each segment's start and end point   IMU 样本在每个段的起点和终点处进行插值
            ...
            // Reset the association if ufomap has been updated 如果 ufomap 已更新，请重置关联
            ...
            while(outer_iter > 0)
            {
                ...
                // Calculate the downsampling rate at each depth            计算每个深度的下采样率
                ...
                // Redo associations and loop if optimization iterations exceed the threshold  如果优化迭代超过阈值，则重做关联并循环
    ...
    void InitSensorData(slict::FeatureCloud::ConstPtr &packet)
    {
        ...
        if (!LIDAR_INITED)
        {
            ...
            if(IMU_INITED)
            {   
                // Downsample the cached kf data  在每个体素保留一个最接近体素中心的点，代替体素中所有点
                pcl::UniformSampling<PointXYZIT> downsampler;
                downsampler.setRadiusSearch(leaf_size);
                downsampler.setInputCloud(kfCloud0);
                downsampler.filter(*kfCloud0);

                // Admit the pointcloud to buffer  接纳点云缓冲
                      ...
                      slict::FeatureCloud::ConstPtr &packet, bool regularize_imu)
    {
        ...
        // Extract and regularize imu data at on the latest buffer在最新的缓冲区上提取和规范化 imu 数据
        ImuSequence newImuSequence;
        ExtractImuData(newImuSequence, packet, regularize_imu); // Only select the primary IMU at this stage  在此阶段仅选择主 IMU

        // Extract subsequence of imu sample data, interpolate at sub steps  提取imu样本数据的子序列，分步插值

        if(timeStepDeque.size() == 1)
        {
            // Duplicate the first sample for continuity 复制第一个样本以实现连续性
            newImuSequence.push_front(newImuSequence.front());
            newImuSequence.front().t = timeStepDeque.front().front().start_time;
        }
        else
            // Borrow the last sample time in the previous interval for continuity 借用上一个区间的最后一个采样时间来保持连续性
            newImuSequence.push_front(imuBundleDeque.rbegin()[1].back().back());

        // Extract the samples in each sub interval   提取每个子区间的样本         
    ...
    void LIOOptimization(slict::OptStat &report)
    {
        ...
        struct StateID
        {
            ...
            int step_idx;        // Time step that the state belongs to  状态所属的时间步
            int segm_idx;     // Time segment in the step that the state belongs to  状态所属步骤中的时间段
            int state_idx;      // Index of the state  状态索引
            int end_type;     // The end of the seg: '0' for start, '1' for final.  段的结尾：“0”表示开始，“1”表示结束。
                                            // Only the first should stateID should be 0         只有第一个应该stateID应该是0
        };
        ...
        // Load values from state to params  将值从状态加载到参数
        for(int i = 0; i < WINDOW_SIZE; i++)
        {
            for(int j = 0; j < SwTimeStep[i].size(); j++)
            {
                // Add the start time of the first segment in the first step   在第一步添加第一个段的开始时间
                if (i == 0 && j == 0)
                {
                    double timeInstance = SwTimeStep[i][j].start_time;
                    timeToIdx[timeInstance] = StateID(i, j, PARAM_POSE.size(), 0);

                    // Intialize the pose states  初始化姿势状态
                ...
                // Add the end time of each segment   添加每个段的结束时间
                               ...
                               deque<LidarCoef> &CloudCoef, map<int, int> &stat)
    {
        ...
        {
            // Initialize the coefficent buffer  初始化系数缓冲区       点数 * 体素深度
            CloudCoef = deque<LidarCoef>(pointsCount * surfel_query_depth);

            // Check the fitness of points with respect to each surfel  检查关于每个面元的点的适合度
            #pragma omp parallel for num_threads(MAX_THREADS)
        ...
        for(auto &point : CloudDeskewedDS->points)
        {
            ...
            {
                // printf(KRED "Invalid surf point!: %f, %f, %f\n" RESET, pointInB.x, pointInB.y, pointInB.z);无效点将数值置零
                pointInB.x = 0; pointInB.y = 0; pointInB.z = 0; pointInB.intensity = 0;
                continue;
            }

            // Query the surfel map with predicates 用预测值查询surfelmap,在这个阶段，我们仍然搜索低平面度的面元，以避免在狭窄的通道中迷失方向
            // 初始化ufomap相关的参数设置
            ...
            auto pred = ufopred::HasSurfel()
                     ...
                     && ufopred::Intersects(ufo::geometry::Sphere(ufo::map::Point3(pointInW.x, pointInW.y, pointInW.z), surfel_intsect_rad)); // 查找半径
            ...
            for (auto const &node : Map.query(pred))
            {
                ...
                // If node depth is higher than the second level, only admit highly planar ones 如果节点深度高于第二层，只接受高度平面的
                // 在第二层保持低平面度面元作为丢失跟踪的备份
                ...
                double d2pln = fabs(norm.dot(Vector3d(pointInW.x, pointInW.y, pointInW.z) - mean));     //点到surfel的距离
            ...
            for (int depth = 0; depth < surfel_query_depth; depth++)
            {
                // Write down the d2p for the original point  记下原点的 d2p
                ...
                // Weightage based on how close the point is to the plane  基于点到平面的距离的权重
                if (score > score_min)
                {
